...
\urlstyle{same}
\hypersetup{
  pdftitle={Health Environment R-Spatial Toolkit},
...
\title{Health Environment R-Spatial Toolkit}
...
analysis} for opioid environment applications that will allow
researchers, analysts, and practitioners to support their communities
with better data analytics and visualization services.
...
mapping, and understanding the opioid risk environment.
...
We assume a basic knowledge of R and coding languages for these
toolkits. For most of the tutorials in this toolkit, you'll need to have
R and RStudio downloaded and installed on your system. You should be
able to install packages, know how to find the address to a folder on
your computer system, and have very basic familiarity with R. If you are
new to R, we recommend the following
\href{https://learn.datacamp.com/courses/free-introduction-to-r}{intro-level
tutorials} provided through
\href{https://rspatial.org/intr/1-introduction.html}{installation}
...
the NIH HEAL Initiative under award number U2CDA050098. The contents of
this publication are solely the responsibility of the authors and do not
necessarily represent the official views of the NIH, the NIH HEAL
Initiative, or the participating sites.
...
{\centering \includegraphics[width=10.67in,height=\textheight,keepaspectratio]{img/vector_raster_data_2.jpg}
...
{\centering \includegraphics[width=3.41in,height=\textheight,keepaspectratio]{img/park.jpg}
...
information and geographic boundaries. Remember, this type of data (a
shapefile) is actually comprised of multiple files. All need to be
present in order to read correctly.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Chi\_tracts }\OtherTok{=} \FunctionTok{st\_read}\NormalTok{(}\StringTok{"data/geo\_export\_aae47441{-}adab{-}4aca{-}8cb0{-}2e0c0114096e.shp"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\section{Non-Spatial \& Spatial Views}\label{non-spatial-spatial-views}

Always inspect data when loading in. First we look at a non-spatial
view.
...
We can use a \emph{baseR} function to view the spatial dimension. The
...
\pandocbounded{\includegraphics[keepaspectratio]{intro_files/figure-pdf/spatial-view-1.pdf}}

\section{Spatial Data Structure}\label{spatial-data-structure}
...
\section{Exploring Coordinate Reference
Systems}\label{exploring-coordinate-reference-systems}
...
plot, we use baseR again but with some paremeter updates. Finally, we
...
\pandocbounded{\includegraphics[keepaspectratio]{intro_files/figure-pdf/unnamed-chunk-5-1.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#st\_write(Chi\_tracts.3435, "ChiTracts{-}3435.gpkg")}
\end{Highlighting}
\end{Shaded}
...
\section{Mapping the Basics}\label{mapping-the-basics}
...
\pandocbounded{\includegraphics[keepaspectratio]{intro_files/figure-pdf/unnamed-chunk-9-1.pdf}}
...
resources. This files can be found
...
Then load the libraries for use. \emph{Note:} The messages you see about
GEOS, GDAL, and PROJ refer to software libraries that allow you to work
with spatial data.
...
\texttt{tidygeocoder} package. It uses mutliple geocoding services,
providing the user with an option to choose. It also provides the option
to use a \texttt{cascade} method which queries other geocoding services
incase the default method fails to provide coordinates.
...
may require more complex data pipelines. The default method used here is
via \href{https://geocoding.geo.census.gov/}{US Census geocoder}, which
allows around 10,000 addresses to be geocoded at once. Others have
varying daily limits. The Google Maps API and ESRI Geocoding service are
additional high-quality geocoding services with varying cost associated.
...
\NormalTok{geoCodedClinics }\OtherTok{\textless{}{-}}\NormalTok{ methadoneClinics }\SpecialCharTok{\%\textgreater{}\%} 
                            \FunctionTok{geocode}\NormalTok{(}\AttributeTok{address =} \StringTok{\textquotesingle{}fullAdd\textquotesingle{}}\NormalTok{, }
                                    \AttributeTok{lat =}\NormalTok{ latitude, }\AttributeTok{long =}\NormalTok{ longitude, }\AttributeTok{method =} \StringTok{\textquotesingle{}osm\textquotesingle{}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
The code worked for all addresses except the first two. We already
resolved the \texttt{4545\ North\ Broadway\ St.}address above but here
in the dataframe we get NAs. It is pointing to some issue with the
string input. These were missed in the previous quality check, but give
us a clue to the types of errors we could see if geocoding more
addresses. Unfortunately, such quirks are common across geocoding
services in R and we just have to handle them. We manually update the
full address strings to get apprpriate coordinates.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{methadoneClinics[}\DecValTok{1}\NormalTok{,}\StringTok{\textquotesingle{}fullAdd\textquotesingle{}}\NormalTok{] }\OtherTok{\textless{}{-}} \StringTok{\textquotesingle{}4453 North Broadway St.,Chicago IL 60640\textquotesingle{}}
\NormalTok{methadoneClinics[}\DecValTok{2}\NormalTok{,}\StringTok{\textquotesingle{}fullAdd\textquotesingle{}}\NormalTok{] }\OtherTok{\textless{}{-}} \StringTok{\textquotesingle{}4545 North Broadway St.,Chicago IL 60640\textquotesingle{}}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
Query completed in: 31.2 seconds
\end{verbatim}
...
First we switch the \texttt{tmap} mode to view so we can look at the
points with a live basemap layer. Uncomment, and run the code locally.
...
Next, we plot our points as dots, and add the basemap. Uncomment, and
run the code locally.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#tm\_shape(methadoneSf) + tm\_dots() + tm\_basemap("OpenStreetMap")}
\end{Highlighting}
\end{Shaded}
...
Reading layer `methadone_clinics' from data source 
  `/Users/maryniakolak/Code/opioid-environment-toolkit/HealthyPlacesToolkit/data/methadone_clinics.shp' 
  using driver `ESRI Shapefile'
...
Reading layer `chicago_zips' from data source 
  `/Users/maryniakolak/Code/opioid-environment-toolkit/HealthyPlacesToolkit/data/chicago_zips.shp' 
  using driver `ESRI Shapefile'
...
Reading layer `boundaries_chicago' from data source 
  `/Users/maryniakolak/Code/opioid-environment-toolkit/HealthyPlacesToolkit/data/boundaries_chicago.geojson' 
...
Geometry type: MULTIPOLYGON
Dimension:     XY
Bounding box:  xmin: -88.06058 ymin: 41.73452 xmax: -87.58209 ymax: 42.04052
Geodetic CRS:  WGS 84
...
\FunctionTok{tm\_shape}\NormalTok{(areas) }\SpecialCharTok{+} \FunctionTok{tm\_borders}\NormalTok{(}\AttributeTok{alpha =} \FloatTok{0.4}\NormalTok{) }\SpecialCharTok{+} 
  
  \DocumentationTok{\#\# 2nd layer (overlay)}
  \FunctionTok{tm\_shape}\NormalTok{(metClinics) }\SpecialCharTok{+} \FunctionTok{tm\_dots}\NormalTok{(}\AttributeTok{size =} \FloatTok{0.4}\NormalTok{, }\AttributeTok{col=}\StringTok{"red"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}
...
(instead of 'col'), and 'col' for the outlines (instead of 'border.col').[v3->v4] `tm_borders()`: use `fill_alpha` instead of `alpha`.
\end{verbatim}
...
code areas, then providers, and then finally the buffers. We use red to
color clinics, and blue to color buffers.
...
\FunctionTok{tm\_shape}\NormalTok{(areas}\FloatTok{.3435}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{tm\_borders}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{tm\_shape}\NormalTok{(metClinics}\FloatTok{.3435}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{tm\_dots}\NormalTok{(}\AttributeTok{col =} \StringTok{"red"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{tm\_shape}\NormalTok{(metClinic\_buffers) }\SpecialCharTok{+} \FunctionTok{tm\_borders}\NormalTok{(}\AttributeTok{col =} \StringTok{"blue"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{buffers_files/figure-pdf/unnamed-chunk-12-1.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Map Housing Buffers}
\FunctionTok{tm\_shape}\NormalTok{(areas) }\SpecialCharTok{+} \FunctionTok{tm\_borders}\NormalTok{(}\AttributeTok{alpha =} \FloatTok{0.6}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{tm\_shape}\NormalTok{(metClinic\_buffers) }\SpecialCharTok{+} \FunctionTok{tm\_fill}\NormalTok{(}\AttributeTok{col =} \StringTok{"blue"}\NormalTok{, }\AttributeTok{alpha =}\NormalTok{ .}\DecValTok{4}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{tm\_borders}\NormalTok{(}\AttributeTok{col =} \StringTok{"blue"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{tm\_shape}\NormalTok{(metClinics}\FloatTok{.3435}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{tm\_dots}\NormalTok{(}\AttributeTok{col =} \StringTok{"red"}\NormalTok{,  }\AttributeTok{size =} \FloatTok{0.2}\NormalTok{) }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{tm\_shape}\NormalTok{(areas) }\SpecialCharTok{+}  \FunctionTok{tm\_borders}\NormalTok{()}\SpecialCharTok{+}
  \FunctionTok{tm\_shape}\NormalTok{(unionBuffers) }\SpecialCharTok{+} \FunctionTok{tm\_fill}\NormalTok{(}\AttributeTok{col =} \StringTok{"blue"}\NormalTok{, }\AttributeTok{alpha =}\NormalTok{ .}\DecValTok{2}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{tm\_borders}\NormalTok{(}\AttributeTok{col =} \StringTok{"blue"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{tm\_shape}\NormalTok{(metClinics}\FloatTok{.3435}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{tm\_dots}\NormalTok{(}\AttributeTok{col =} \StringTok{"red"}\NormalTok{, }\AttributeTok{size =} \FloatTok{0.4}\NormalTok{) }
\end{Highlighting}
\end{Shaded}
...
And then leverage tmap parameter specifications to further customize the
a map showing multiple buffers. Here, we add the City of Chicago
boundary and soften the zip code boundaries. We add a bounding box for
the first zip code layer, so that the whole map is centered around the
city boundary (even those the zip codes are layered first). We adjust
the transparency of the buffer fills, use different colors, and adjust
borders to make the visuals pop. We use the \texttt{tmap\_layout}
function to take away the frame, add and position a title. Explore the
\texttt{tmap} documentation further to find additional options for
legends and more. To find color options in R, there are multiple guides
online (like
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{tm\_shape}\NormalTok{(areas, }\AttributeTok{bbox=}\NormalTok{cityBoundary) }\SpecialCharTok{+} \FunctionTok{tm\_borders}\NormalTok{(}\AttributeTok{alpha =} \FloatTok{0.2}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{tm\_shape}\NormalTok{(cityBoundary) }\SpecialCharTok{+} \FunctionTok{tm\_borders}\NormalTok{(}\AttributeTok{lwd =} \FloatTok{1.5}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{tm\_shape}\NormalTok{(metClinic\_2mbuffers) }\SpecialCharTok{+} \FunctionTok{tm\_fill}\NormalTok{(}\AttributeTok{col =} \StringTok{"gray10"}\NormalTok{, }\AttributeTok{alpha =}\NormalTok{ .}\DecValTok{4}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{tm\_borders}\NormalTok{(}\AttributeTok{col =} \StringTok{"dimgray"}\NormalTok{, }\AttributeTok{alpha =}\NormalTok{ .}\DecValTok{4}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{tm\_shape}\NormalTok{(metClinic\_buffers) }\SpecialCharTok{+} \FunctionTok{tm\_fill}\NormalTok{(}\AttributeTok{col =} \StringTok{"gray90"}\NormalTok{, }\AttributeTok{alpha =}\NormalTok{ .}\DecValTok{4}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{tm\_borders}\NormalTok{(}\AttributeTok{col =} \StringTok{"darkslategray"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{tm\_shape}\NormalTok{(metClinics}\FloatTok{.3435}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{tm\_dots}\NormalTok{(}\AttributeTok{col =} \StringTok{"red"}\NormalTok{,  }\AttributeTok{size =} \FloatTok{0.2}\NormalTok{) }\SpecialCharTok{+}   
  \FunctionTok{tm\_layout}\NormalTok{(}\AttributeTok{main.title =} \StringTok{"Methadone Clinic Service Areas in Chicago"}\NormalTok{,}
...
-- tmap v3 code detected -------------------------------------------------------
[v3->v4] `tm_borders()`: use `fill_alpha` instead of `alpha`.[v3->v4] `tm_polygons()`: use `fill_alpha` instead of `alpha`.[v3->v4] `tm_borders()`: use `fill_alpha` instead of `alpha`.[v3->v4] `tm_polygons()`: use `fill_alpha` instead of `alpha`.[v3->v4] `tm_layout()`: use `tm_title()` instead of `tm_layout(main.title = )`[plot mode] fit legend/component: Some legend items or map compoments do not
...
merged 1-mile buffer service areas. We add labels for zip codes using
the \texttt{tm\_text} parameter, and adjust the size. The resulting map
lets us zoom and out to explore the data. Clicking on a point will give
...
\CommentTok{\#tm\_shape(areas) +  tm\_borders(alpha = 0.5, col="gray")+ tm\_text\#("GEOID10", size = 0.7) +}
\CommentTok{\#  tm\_shape(cityBoundary) + tm\_borders() +}
\CommentTok{\#  tm\_shape(unionBuffers) + tm\_fill(col = "blue", alpha = .2) + \#tm\_borders(col = "blue") +}
\CommentTok{\#  tm\_shape(metClinics.3435) + tm\_dots(col = "red") }
\end{Highlighting}
\end{Shaded}
...
Reading layer `methadoneClinics_1mi' from data source 
  `/Users/maryniakolak/Code/opioid-environment-toolkit/HealthyPlacesToolkit/data/methadoneClinics_1mi.shp' 
  using driver `ESRI Shapefile'
...
\section{Clean \& Merge Data}\label{clean-merge-data}

First, let's inspect COVID case data. What information do we need from
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{COVID.sub }\OtherTok{\textless{}{-}}\NormalTok{ COVID[, }\FunctionTok{c}\NormalTok{(}\StringTok{"ZIP.Code"}\NormalTok{, }\StringTok{"Case.Rate...Cumulative"}\NormalTok{)]}
\FunctionTok{head}\NormalTok{(COVID.sub)}
\end{Highlighting}
\end{Shaded}
...
Before merging, we need to first identify the \textbf{geographic
identifier} we would like to merge on. It is the field ``ZIP.Code'' in
our subset. What about the zip code file, which we will be merging to?
...
Aha -- in this dataset, the zip is identified as either the ZCTA5CE10
field or GEOID10 field. Not that we are actually working with 5-digit
ZCTA fields, not 9-digit ZIP codes\ldots{} We decide to merge on the
GEOID10 field. To make our lives easier, we'll generate a duplicate
field in our subset with a new name, GEOID10, to match. We also convert
from the factor structure to a character field to match the data
structure of the master zip file.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{COVID.sub}\SpecialCharTok{$}\NormalTok{GEOID10}\OtherTok{\textless{}{-}} \FunctionTok{as.character}\NormalTok{(COVID.sub}\SpecialCharTok{$}\NormalTok{ZIP.Code)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{zipsMerged }\OtherTok{\textless{}{-}} \FunctionTok{merge}\NormalTok{(chicagoZips, COVID.sub, }\AttributeTok{by =} \StringTok{"GEOID10"}\NormalTok{)}
...
Geometry type: MULTIPOLYGON
Dimension:     XY
Bounding box:  xmin: -87.63396 ymin: 41.88083 xmax: -87.6129 ymax: 41.88893
Geodetic CRS:  WGS 84
...
\FunctionTok{tm\_shape}\NormalTok{(zipsMerged) }\SpecialCharTok{+}
  \FunctionTok{tm\_polygons}\NormalTok{(}\StringTok{"Case.Rate...Cumulative"}\NormalTok{, }\AttributeTok{style=}\StringTok{"quantile"}\NormalTok{, }\AttributeTok{pal=}\StringTok{"BuPu"}\NormalTok{,}
              \AttributeTok{title =} \StringTok{"COVID Case Rate"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{tm\_shape}\NormalTok{(buffers) }\SpecialCharTok{+} \FunctionTok{tm\_borders}\NormalTok{(}\AttributeTok{col =} \StringTok{"blue"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{tm\_shape}\NormalTok{(methadoneSf) }\SpecialCharTok{+} \FunctionTok{tm\_dots}\NormalTok{(}\AttributeTok{col =} \StringTok{"black"}\NormalTok{, }\AttributeTok{size =} \FloatTok{0.2}\NormalTok{) }
\end{Highlighting}
\end{Shaded}
...
`tm_scale_intervals()`.
i Migrate the argument(s) 'style' to 'tm_scale_intervals(<HERE>)'[v3->v4] `tm_polygons()`: migrate the argument(s) related to the legend of the
visual variable `fill` namely 'title' to 'fill.legend = tm_legend(<HERE>)'[tm_polygons()] Argument `pal` unknown.
\end{verbatim}
...
\begin{Highlighting}[]
\FunctionTok{tm\_shape}\NormalTok{(zipsMerged) }\SpecialCharTok{+}
  \FunctionTok{tm\_fill}\NormalTok{(}\StringTok{"Case.Rate...Cumulative"}\NormalTok{, }\AttributeTok{style=}\StringTok{"quantile"}\NormalTok{, }\AttributeTok{n=}\DecValTok{6}\NormalTok{, }\AttributeTok{pal=}\StringTok{"{-}RdYlGn"}\NormalTok{,}
              \AttributeTok{title =} \StringTok{"COVID Case Rate"}\NormalTok{,}\AttributeTok{alpha =} \FloatTok{0.8}\NormalTok{) }\SpecialCharTok{+} 
  \FunctionTok{tm\_borders}\NormalTok{(}\AttributeTok{lwd =} \DecValTok{0}\NormalTok{) }\SpecialCharTok{+} 
  \FunctionTok{tm\_shape}\NormalTok{(buffers) }\SpecialCharTok{+} \FunctionTok{tm\_borders}\NormalTok{(}\AttributeTok{col =} \StringTok{"gray"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{tm\_fill}\NormalTok{(}\AttributeTok{alpha=}\FloatTok{0.5}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{tm\_shape}\NormalTok{(methadoneSf) }\SpecialCharTok{+} \FunctionTok{tm\_dots}\NormalTok{(}\AttributeTok{col =} \StringTok{"black"}\NormalTok{, }\AttributeTok{size =} \FloatTok{0.1}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{tm\_layout}\NormalTok{(}\AttributeTok{main.title =} \StringTok{"Walkable Methadone Service Areas"}\NormalTok{,}
            \AttributeTok{main.title.position =} \StringTok{"center"}\NormalTok{,}
            \AttributeTok{main.title.size =} \DecValTok{1}\NormalTok{,}
            \AttributeTok{frame =} \ConstantTok{FALSE}\NormalTok{)}
\end{Highlighting}
...
\pandocbounded{\includegraphics[keepaspectratio]{linking_files/figure-pdf/unnamed-chunk-10-1.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#write\_sf(zipsMerged, "data/chizips\_COVID.shp")}
\end{Highlighting}
\end{Shaded}
