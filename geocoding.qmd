...
First we switch the `tmap` mode to view so we can look at the points with a live basemap layer. Uncomment, and run the code locally.

```{r view}
#tmap_mode("view")
```

Next, we plot our points as dots, and add the basemap. Uncomment, and run the code locally.
