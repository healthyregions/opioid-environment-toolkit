...
```{r import-libraries-census, warning = FALSE, message=FALSE}
library(sf)
library(tidycensus)
library(tidyverse)
library(tigris)
...
census_api_key("CENSUS_API_KEY", install = TRUE, overwrite=TRUE) # installs the key for future sessions.
```
...
```{r get-census-key, warning = FALSE, message=FALSE}
Sys.getenv("CENSUS_API_KEY")
```

Tip: If you don't see the Census API Key show up here: stop, save, and restart your R-studio session. 
...
```{r census-state, warning = FALSE, message=FALSE}
