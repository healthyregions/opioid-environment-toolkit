...
We will download a data package from OEPS directly, and extract data from the 
`state.csv` file for our analyses, and later merge with geospatial data from the same package, `state-2020-500k-shp`, for mapping and exploration.
...
Our output will be a cleaned county-level CSV file, `final_county_data.csv`, to be used in further research & analyses, as well as several images of maps from our session.
...
First, go to the [OEPS website](https://oeps.healthyregions.org/){target="_blank"}. 

If this is your first time visiting the site, take a moment to explore the [About OEPS](https://oeps.healthyregions.org/about), [Methodology](https://oeps.healthyregions.org/methods), [Data Inventory](https://oeps.healthyregions.org/docs) and [Data Standards](https://oeps.healthyregions.org/dataInclusion) pages to get familiar with site.

Next, under the *Data Access* tab, click  [Download](https://oeps.healthyregions.org/download). On this page, Click *Aggregated Data Packages*.
...
To learn more about each variable, explore the **metadata** folder or navigate to the [**Data Inventory**](https://oeps.healthyregions.org/docs){target="_blank"} page on the OEPS website. 
...
Now, we will read in the state-level CSV file using the `read.csv()` function and inspect its contents. 
...
We use total population as the denominator, because of data availability constraints. Ideally, the denominator would represent the population at risk of the outcome. For HCV-related mortality this could mean the population living with HCV. However, average annual HCV case data were only available as a 5-year pooled estimate covering 2013-2016. Whereas, this tutorial is using pooled mortality data from 2018-2022. 

Thus, when creating health outcome measures, carefully consider which population belongs in the denominator and clearly justify your choice.
...
Now we will identify the rows with missing values for **AvHcvD100K**. The `is.na` function identifies missing values (labeled as NA) in that variable. 
...
In this step, we are creating a new dataframe object called **state_data_filtered** to store the results. `%>%` is a pipe operator that passes the **state_data** object to the next function. The `filter()` function selects rows from a dataframe based on a logical condition. `%in%` checks whether each numerical value appears in the FIPS column, but `!` reverses the condition and means "not".
...
The distribution suggests that HCV mortality is not uniform across states. But it is important to note that summary statistics alone cannot tell us why rates differ. Additional statistical analyses would be needed to examine which factors may account for differences in rates across states. 
...
Now load the shapefile into R using the `st_read()` function and display the geometry column using `plot()` to confirm that it imported correctly. 
...
Next, filter the shapefile to include only the contiguous U.S. This excludes Alaska, Hawaii, and the U.S. territories. This coding chunk follows the same logic in the **Filter the Dataset** section.
...
Now join the **state_data_filtered** dataframe to the filtered shapefile. We join using **HEROP_ID** which serves as a unified geographic identifier for joins. To read more about this variable [click here](https://oeps.healthyregions.org/methods){target="_blank"}. The `left_join()` function combines the two datasets while keeping all rows from the first dataset and adding matching variables from the second dataset. 
...
Finally, map **AvHcvD100K**, which represents the average annual HCV death rate per 100,000 people. For more `tmap` resources, [click here](https://r-tmap.github.io/tmap/reference/){target="_blank"}.
...
## Join OEPS data to additional data
...
Let's keep the OEPS data as is, and instead alter **foodstamps_data** by creating a 5-digit FIPS column. To do so, we need to combine **StateFIPS** with **CountyFIPS** ensuring that **CountyFIPS** does not leave out leading zeros. Here we use the `mutate()` function to create a new variable called **FIPS**. `sprintf` combines the state and county FIPS codes into a 5-digit code:\

- `%02d` formats the state code as a 2-digit integer, adding leading zeros when necessary\
- `%03d` formats the county code as a 3-digit integer, adding leading zeros when necessary\

The `as.numeric()` function converts the 5-digit code into a numeric value. 
...
Finally, let's save our outputs. We will save the final joined county dataset as a CSV, and the county-level maps as images.
...
[Click here](https://oeps.healthyregions.org/){target="_blank"} to learn more about the OEPS ecosystem.

Visit the [Geospatial Consortium & Community of Practice website](https://gccp.healthyregions.org/){target="_blank"} to learn about past and upcoming workshops.

For more on the tmap package [click here](https://r-tmap.github.io/tmap/reference/){target="_blank"}.
